...
class ImageOutput(BaseModel):
    """Final response to the user after using the 'generate_image' tool"""
...
class WebSearchOutput(BaseModel):
    """Final response to the user after using the 'tavily_search_results_json' tool"""

    response: str = Field(
        description="this field is only made of text. It must exclude: hyperlinks that start with 'http://... or 'https://...'"
    )
...
class Response(BaseModel):
    ...
    @validator("output", allow_reuse=True)
    def check_output(cls, v):
        if not v:
            raise ValueError("field 'output' not present")
        print("*" * 100)
        print(v)
        return v
...
class Agent:
    ...
    def __init__(self, _model, _checkpointer, _system="") -> None:
        ...
        graph.add_conditional_edges(
            "llm", self.exist_action, {"action": "action", "__end__": END}
        )
    ...
    def exist_action(self, state: AgentState) -> Literal["action", "__end__"]:
        ...
        if not last_message.tool_calls:  # type: ignore
            return "__end__"
        # If the function Response was called - that is the function that is NOT a tool, but rather the formatted response, so we should NOT continue in that case
        if last_message.tool_calls[0]["name"] == Response.__name__:  # type: ignore
            return "__end__"
...
if __name__ == "__main__":
    ...
    # # query for using web search tool
    # query = "What is the weather in Milan? add some links to check the weather"

    # query for text generation without using any tool.
    query = "Who are you? Do not call any tool!"

    # query for using image generation tool
    query = "a mocking image of the Italian football squad elimination from Switzerland at Euro 2024 and an ironic comment on it"

    # query = "Who won the SuperBowl in 2023? What is the GDP of that state?"
    # query = "What's the weather in Milan and Rome?"
    ...
    print()
    print("\nFINAL OUTPUT")
    print(result["messages"][-1].tool_calls[0]["args"]["output"]["response"])
